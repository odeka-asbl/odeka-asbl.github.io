...
\begin{enumerate}
    ...
    \cmd{NEWS SECTION SWITCH} near the top. Uncomment the two-line
    \file{render:} exclusion inside it.
...
To turn News back on, reverse all four steps (re-comment/uncomment the
opposite way) and render again.

\begin{quote}
\textit{Ask Claude Code: ``Turn the News section off'' or ``Turn the
News section back on'' --- it knows to make all four changes together
and re-render.}
\end{quote}
...
\begin{itemize}
    ...
    \item \term{Photos missing}: the people listed above, plus the
    current PhD Students (Clotaire Boyer, Sarah Danner, Eva Davoine,
    Gabriel Granato, Nic Wicaksono), are all still showing the
    placeholder silhouette. See Section~\ref{sec:projects-todo}'s
    sibling above for cropping conventions once real photos are ready.
...
\begin{itemize}
    ...
    \file{CNAME} file at the root of \file{docs/} naming the domain,
    plus a DNS record set up wherever the domain was purchased (either
    a \cmd{CNAME} record pointing at
    \file{odeka-asbl.github.io}, or \cmd{A} records pointing at
    GitHub's IP addresses, depending on whether it's a subdomain or an
    apex/root domain). Registering the domain itself and configuring
    DNS happens outside this repository, with whoever manages ODEKA's
    domain registrar --- Claude Code can help with the \file{CNAME}
    file and the DNS record values once a domain is picked, but can't
    register a domain or log into a registrar on its own.
