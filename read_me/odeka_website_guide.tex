...
\subsection{Pulling the Latest Changes}
\label{sec:pull}

Before you start editing, make sure your local copy actually matches
what's really on GitHub. \term{Pulling} downloads whatever has changed
there since you last synced --- an edit from another RA, something you
pushed from a different computer, or a change Claude Code made in a
separate session. It's the mirror image of publishing
(Section~\ref{sec:publish}): pull \emph{before} you edit, push
\emph{after}.
...
If you're using GitHub Desktop, the equivalent is the
\textbf{Fetch origin} button at the top --- it relabels itself
\textbf{Pull origin} (with a count of how many commits are waiting)
whenever there's something new to bring down; click it. In VS Code's
Source Control panel, use the circular sync-arrows icon in the bottom
status bar, or the \textbf{...} menu $\to$ \textbf{Pull}.

Skipping this isn't just risky in theory: if someone else's change
reached GitHub after your last pull, your own \cmd{git push} will
simply be \term{rejected} --- Git's way of refusing to silently
overwrite work it doesn't know about, rather than a sign you did
something wrong. The fix is almost always to pull (which will try to
merge the two sets of changes automatically) and then push again. If
Git instead reports an actual \term{merge conflict} --- meaning the
same lines of the same file were edited two different ways --- don't
guess at fixing it by hand; ask Claude Code to resolve it, or ask your
supervisor.

\begin{quote}
\textit{Ask Claude Code: ``Pull the latest changes before we start.''
It's a good habit to say this at the start of any session, especially
if you're not sure whether someone else has edited the site recently.}
\end{quote}
...
\begin{enumerate}
    ...
    \item Pull the latest changes (Section~\ref{sec:pull}), so you're
    starting from what's actually live.
...
A general note before starting: pull the latest changes
(Section~\ref{sec:pull}) before you begin, and after \emph{any} edit
described below, the workflow is always the same steps from Part I ---
render (or preview while you work), check it, then commit and push
...
\begin{itemize}
    \item \term{Photos missing}: several Senior Management and Senior
    Enumerators entries (Junior Ntambue, Gilbert Tshiebue, Jacques
    Kazadi, Max Mbosho, Grégoire Ntumba, Bernard Makinda, Serge Nseya,
    André Kapumbu, Benjamin Badibanga, Théo Kalamba, Jean Kalala),
    plus most of the current PhD Students (Clotaire Boyer, Sarah
    Danner, Eva Davoine, Gabriel Granato, Nic Wicaksono), are still
    showing the placeholder silhouette. Arthur Laroche, the sixth
    current PhD student, already has a real photo --- his entry is the
    model to follow for the rest. See Section~\ref{sec:cropping} for
    cropping conventions once more real photos are ready.
...
The site now has a chosen domain, \url{https://www.odekalab.org}, in
place of the default GitHub Pages address
(\url{https://odeka-asbl.github.io}). On the repository side this is
done: a \file{CNAME} file naming \file{www.odekalab.org} lives in the
\emph{project root} (alongside \file{\_quarto.yml}, not placed by
hand inside \file{docs/} --- Quarto copies loose root-level files like
this into \file{docs/} on every render, the same way it already does
for \file{favicon.ico}, so a root-level copy survives renders reliably
where a \file{docs/}-only one wouldn't), and \file{\_quarto.yml}'s
\texttt{site-url} points at the new domain too (this feeds the
generated \file{sitemap.xml} and social-preview meta tags).

What's still outside this repository: a DNS record needs to exist
wherever \file{odekalab.org} was registered, pointing the
\file{www} subdomain at GitHub Pages --- a \cmd{CNAME} DNS record for
\file{www} targeting \file{odeka-asbl.github.io}. Until that record is
in place and has propagated, \file{www.odekalab.org} won't resolve
(the plain \url{https://odeka-asbl.github.io} address keeps working
throughout). Claude Code can prepare the repository side and state the
exact DNS record needed, but can't log into a registrar or configure
DNS itself.

The separate question this section used to raise --- whether the
address should land on the homepage or on Projects --- is resolved:
Projects and Publications \emph{is} the homepage now (the old
homepage content was replaced, not kept behind a redirect), so the
domain lands on it automatically once DNS is configured.
